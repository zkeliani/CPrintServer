...
//based off of geeksforgeeks algorithm
void insert(struct Node** head, struct Node* new)
{
    struct Node* current;

    if(*head == NULL || (*head)->job.bytes >= new->job.bytes)
    {
        new->next = *head;
        *head = new;
    }
    ...
    {
        current = *head;
        while (current->next != NULL && current->next->job.bytes < new->job.bytes)
        {
...
//based off of geeksforgeeks algorithm
void delete(struct Node **head, struct PrintJob bye)
{
    struct Node* temp = *head, *prev;

    if (temp != NULL && temp->job.bytes == bye.bytes)
    {
        *head = temp->next;
        free(temp);
        return;
    ...
    while (temp != NULL && temp->job.bytes != bye.bytes)
    {
        prev = temp;
        temp = temp->next;
    }
